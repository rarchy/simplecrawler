...
FetchQueue.prototype.add = function (queueItem, force, callback) {
    ...
    function addToQueue() {
        ...
        queueItem.id = queueItem.wasRedirected ? queue._oldestUnfetchedIndex + 1 : queue.length;
        queueItem.status = "queued";
        if (queueItem.wasRedirected) {
            delete queueItem.wasRedirected;
            queue.splice(queueItem.id, 1, queueItem);
        } else {
...
FetchQueue.prototype.freeze = function (filename, callback) {
    ...
    queue.forEach(function (item) {
        if (item.fetched !== true) { item.status = "queued"; }
    });
 ...
 * @param {Function} callback Gets a single `error` parameter. If the operation was successful, this parameter will be `null`.
 */
FetchQueue.prototype.defrost = function (fileData, callback) {
    var queue = this, defrostedQueue = [];

    try {
        defrostedQueue = fileData
    } catch (error) {
